...
配置遵循 [Grok Build 官方 Hooks 协议](https://docs.x.ai/build/features/hooks)，注册 `SessionStart`、`UserPromptSubmit`、`PreToolUse`、`PostToolUse`、`PostToolUseFailure`、`Notification`、`Stop`、`StopFailure` 和 `SessionEnd`。Grok 把事件 JSON 写入隐藏的 `<bridge> __hook` stdin；该进程完整读取输入、提取有界状态字段，再通过现有本地 IPC 更新对应 Session。`create/list/show/send/write/resize` 返回的 Session JSON 会携带 `activity`、`hook_event`、`hook_at_ms`、`tool_name` 和 `waiting_reason`，Codex 因此可以直接判断工作中、等待输入和本轮完成；`StopFailure` 也不会让状态永久停在 Running。PTY 仍是输入、终端输出和进程终止的唯一控制通道，Hook 不进入 `read` 的 byte cursor。
...
- owner 会显示在 JSON 和 WebUI 中，不要把用户名密码或其他秘密用作 owner；
